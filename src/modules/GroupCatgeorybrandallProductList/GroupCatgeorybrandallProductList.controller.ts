...
export const getCategoryGroupMappings = catchAsync(
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (invalid || ids.length === 0) {
        throw new BusinessError(
          "Valid category_group_id(s) required",
          ErrorCodes.BUSINESS_RULE_VIOLATION,
        );
      }
 ...
 */
export const getCategoryBrandStructure = catchAsync(
  async (req: AuthRequest, res: Response) => {
    try {
      const raw = req.query.category_group_id;
 ...
 * /brand-products/5
 */
export const getBrandWithProducts = catchAsync(
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!categoryId || !brandId || isNaN(categoryId) || isNaN(brandId)) {
        throw new BusinessError(
          "Valid category_id and brand_id are required",
          ErrorCodes.BUSINESS_RULE_VIOLATION,
        );
      }
      ...
      if (!data) {
        throw new BusinessError(
          "Brand not found or no products available",
          ErrorCodes.BUSINESS_RULE_VIOLATION,
        );
      }
...
export const getProductDynamicFields = catchAsync(
  async (req: AuthRequest, res: Response) => {
    try {
